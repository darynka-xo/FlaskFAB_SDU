...
from .views import (
    ...
    UserStatsChartView,
    ViewMenuModelView, LoginUserLogView,
)
...
class BaseSecurityManager(AbstractSecurityManager):
    ...
    loginlogmodelview = LoginUserLogView
    ...
    def register_views(self):
        ...
        if self.loginlogmodelview:
            self.appbuilder.add_view(
                self.loginlogmodelview,
                "Login log",
    ...
    def auth_user_db(self, username, password):
        ...
        if user is None or (not user.is_active):
            ...
            return None
        elif user.is_user_blocked_by_bruteforce:
            flash(_("Your account is temporarily blocked due to multiple failed login attempts."), 'warning')
            return None
    ...
    def auth_user_ldap(self, username, password):
        ...
        # If user is blocked by bruteforce, go away
        if user.is_user_blocked_by_bruteforce:
            flash(_("Your account is temporarily blocked due to multiple failed login attempts."), 'warning')
            return None
    ...
    def before_request():
        ...
        if current_user and current_user.is_authenticated and not request.path.startswith('/static'):
            if current_user.user_should_agree_nda \
                    and not request.path.startswith('/agree_nda') \
                    and not request.path.startswith('/api/agree_nda'):
                return redirect('/agree_nda')

            if current_user.should_user_change_password and not current_user.user_should_agree_nda \
                    and not request.path.startswith('/change_password') \
                    and not request.path.startswith('/api/change_password'):
