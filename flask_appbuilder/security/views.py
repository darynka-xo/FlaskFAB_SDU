...
from flask_appbuilder.security.utils import login_user
...
class LoginUserLogView(ModelView):
    route_base = "/loginlog"
    base_permissions = ["can_list", "can_mylog"]
    list_title = lazy_gettext("List of Registration Requests")
    show_title = lazy_gettext("Show Registration")
    list_columns = ["user", "dttm", "addr", "status", "reason"]
    base_order = ["dttm", "desc"]

    @expose("/mylog")
    @has_access
    def mylog(self) -> WerkzeugResponse:
        self.update_redirect()
        self.list_columns = ["dttm", "addr", "status", "reason"]
        self.search_columns = self.list_columns
        self._base_filters = self.datamodel.get_filters().add_filter(
            "user_id", self.datamodel.FilterEqual, g.user.id
        )
        try:
            widgets = self._list()
    ...
    @expose("/list")
    @has_access
    def list(self) -> WerkzeugResponse:
        self.update_redirect()
        self.list_columns = ["user", "dttm", "addr", "status", "reason"]
        self.search_columns = self.list_columns
