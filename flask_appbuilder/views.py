...
from flask_appbuilder.exceptions import FABException, PasswordComplexityValidationError
...
class IndexView(BaseView):
    ...
    @expose("/agree_nda")
    def agree_nda_page(self):
        if not g.user.user_should_agree_nda:
            return redirect(self.appbuilder.get_url_for_index)

        return self.render_template('appbuilder/agree_nda.html', appbuilder=self.appbuilder)

    @expose("/change_password")
    def change_password_page(self):
        if not g.user.is_authenticated:
            return jsonify(message=_("Not authenticated")), 401

        if not g.user.should_user_change_password:
            return redirect(self.appbuilder.get_url_for_index)

        return self.render_template('appbuilder/change_password.html', appbuilder=self.appbuilder)

    @expose('/api/agree_nda', methods=['POST'])
    def agree_nda(self):
        user = self.appbuilder.session.query(self.appbuilder.sm.user_model).filter_by(id=g.user.id).one_or_none()
        user.nda_agreed = True

        self.appbuilder.session.add(user)
        self.appbuilder.session.commit()
        return jsonify(success=True)

    @expose('/api/change_password', methods=['POST'])
    def change_password(self):
        ...
        if not new_password:
            return jsonify(message=_("No new password provided")), 400

        if check_password_hash(user.password, new_password):
            return jsonify(message=_("New password cannot be the same as the old password")), 400

        password_complexity_validator = self.appbuilder.get_app.config.get(
            "FAB_PASSWORD_COMPLEXITY_VALIDATOR", None
        )
        password_complexity = self.appbuilder.get_app.config.get("FAB_PASSWORD_COMPLEXITY_ENABLED", False)

        if password_complexity and password_complexity_validator is not None:
            try:
                password_complexity_validator(new_password)
        ...
        return jsonify(message=_("Password successfully changed")), 200
